...
def createPerformanceWindow():
    def submit():
        sid = sid_entry.get()
        subjectid = subjectid_entry.get() #changed subject to subject_id
        ...
        comments = comments_entry.get() # remove comments field since it's auto generated
...
def createTeacherWindow():
    def submit():
        ...
        courseid = courseid_entry.get() # added 
        if tid or password or fname or lname or address or employ_date or qualifications or email or department == "":
            messagebox.showerror("Error", "All fields must be filled")
        else:
            teacherinfo(tid, password, user_type, fname, lname, address, employ_date, qualifications, departmentid, courseid)
            pass
